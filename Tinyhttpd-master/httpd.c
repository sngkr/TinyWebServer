...
void accept_request(void *arg) {
  ...
  if (stat(path, &st) == -1) {
    ...
    /*
    <sys/stat.h>
    st_mode 32位 linux用了低16位
    file type 属性： 15 ~ 12 位
    linux操作系统一共有7种文件，所以3位就可以表示完，
    普通文件（regular file）
    目录（directory）
    ...
    */
    ...
    // #define S_IRWXO 00007	/* mask for permissions for others (not in
    // group)
    // */ #define S_IROTH 00004	/* others have read permission */ #define
    // S_IWOTH 00002	/* others have write permission */ #define S_IXOTH 00001
    // /* others have execute permission */

    // 判断是否为目录
    if ((st.st_mode & S_IFMT) == S_IFDIR) strcat(path, "/index.html");
    //文件默认有执行权限的话，自动解析成cgi程序，否则报错
    if ((st.st_mode & S_IXUSR) || (st.st_mode & S_IXGRP) ||
        (st.st_mode & S_IXOTH))
      cgi = 1;
    //   判断是执行一个cgi程序还是返回一个内容给客户端
    if (!cgi)
...
int startup(u_short *port) {
  ...
  //   允许端口复用 防止bind：Address already in use
